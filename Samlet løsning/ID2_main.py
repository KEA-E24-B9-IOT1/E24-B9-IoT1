...
# Raffiner hvilken data vi henter og gemmer hvornår
    if hw.gps.receive_nmea_data():
        ...
        if printable==True:
            hw.display(0,  0, f"Bat%:{data.get('Batt%'):.1f}")
            hw.display(10, 0, f"Temp:{data.get('Temp'):.1f}")
            hw.display(0,  1, f"La/Lo:{float(data.get('La')):.3f}")
            hw.display(0, 1, f"La:{float(data.get('Lo')):.3f}")
